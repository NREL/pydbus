...
def start_DBus():
    ...
    if platform.system() == "Darwin":
        ...
        raise NotImplementedError(
            "Linux is currently not tested. Please contact developer for support."
        )

    _p = subprocess.Popen(
        "./DBUS_CMD",
        cwd=os.path.abspath(
            os.path.join(CURRENT_DIRECTORY, "library/{osname}/".format(osname=osname))
        ),
    )
...
class DBus(object):
    ...
    def __init__(self, name, variables={}, type="master", should_assert=True):
        ...
        for x in self._variables:
            assert x.type in VariableTypes, "Unknown type received {} for {}".format(
                x.type, x.name
            )

        if type is "master":
            self._type = self._length_in_bytes(ComponentTypes.MASTER.value)  # Master
        elif type is "slave":
            self._type = self._length_in_bytes(ComponentTypes.SLAVE.value)  # Slave
        else:
            raise NotImplementedError("Unable to set type of DBus client")
    ...
    def _connect(self):
        ...
        data = self._recv(
            ft, assertion=self._assertion, decode=False
        )  # connection status
    ...
    def _request_component_information(self):
        ...
        assert (
            data != Status.ERROR.value
        ), "Received ERROR status from DBus. Please check the error register."
        ...
        for component in components:
            component_variables[component]["variables"] = variable_names[
                0 : component_variables[component]["size"]
            ]
        ...
        for component in components:
            ...
            for _ in variables:
                component_variables[component]["types"].append(
    ...
    def _subscribe_component_variables(self, names, variables):
        ...
        if isinstance(names, (str, bytes, bytearray)) or isinstance(
        ...
        data = (
            ...
            + name_of_active_components
            + number_of_variables_per_component
            + name_of_variables_per_components
        )
    ...
    def _update_component_variables(self, data):
        ...
        for i, datum in enumerate(data):
            b = (
                ...
                + bytearray(
                    struct.pack(
                        self.variable_struct_mapping[self._variables[i].type], datum
                    )
                )[::-1]
            )
    ...
    def _recv(self, ft, assertion=False, decode=True):
        ...
        if assertion is True:
            ...
            ), "Something went wrong with action {rft}".format(
                rft=FunctionTypes(ft).name
            )
    ...
    def _decode(cls, data, ft):
        ...
        size = cls._sizeof(
            *data[0:4]
        )  # TODO: Fix return size calculation and use for assertion
        logger.debug("Size of data %s is interpreted as %s", data, size)
        logger.info("data = %s, type = %s, len = %s", data, type(data), len(data))
        assert (
            data[4] == ft
        ), "Unrecognized return type for action {rft}, got {ft}".format(
            rft=FunctionTypes(ft).name, ft=data[4]
        )
    ...
    def _length_in_bytes(length, number_of_bytes=1):
        header_size = bytes(
            [
                length >> i & 0xff
                for i in (
                    8 * (number_of_bytes - j - 1) for j in range(0, number_of_bytes)
                )
            ]
        )
        return header_size
